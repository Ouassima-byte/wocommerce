 ...
 */
class EanUpc extends \WPDeskFIVendor\Mpdf\Barcode\AbstractBarcode implements \WPDeskFIVendor\Mpdf\Barcode\BarcodeInterface
{
    /**
     * @param string $code
     * @param int $length
    ...
    private function init($code, $length)
    {
        if (\preg_match('/[\\D]+/', $code)) {
            throw new \WPDeskFIVendor\Mpdf\Barcode\BarcodeException('Invalid EAN UPC barcode value');
        }
        $upce = \false;
        $checkdigit = \false;
        if ($length == 6) {
        ...
        if ($upce) {
            ...
            if ($invalidUpce) {
                throw new \WPDeskFIVendor\Mpdf\Barcode\BarcodeException('UPC-A cannot produce a valid UPC-E barcode');
            }
        }
        ...
        $upceParities = [];
        $upceParities[0] = ['0' => ['B', 'B', 'B', 'A', 'A', 'A'], '1' => ['B', 'B', 'A', 'B', 'A', 'A'], '2' => ['B', 'B', 'A', 'A', 'B', 'A'], '3' => ['B', 'B', 'A', 'A', 'A', 'B'], '4' => ['B', 'A', 'B', 'B', 'A', 'A'], '5' => ['B', 'A', 'A', 'B', 'B', 'A'], '6' => ['B', 'A', 'A', 'A', 'B', 'B'], '7' => ['B', 'A', 'B', 'A', 'B', 'A'], '8' => ['B', 'A', 'B', 'A', 'A', 'B'], '9' => ['B', 'A', 'A', 'B', 'A', 'B']];
        $upceParities[1] = ['0' => ['A', 'A', 'A', 'B', 'B', 'B'], '1' => ['A', 'A', 'B', 'A', 'B', 'B'], '2' => ['A', 'A', 'B', 'B', 'A', 'B'], '3' => ['A', 'A', 'B', 'B', 'B', 'A'], '4' => ['A', 'B', 'A', 'A', 'B', 'B'], '5' => ['A', 'B', 'B', 'A', 'A', 'B'], '6' => ['A', 'B', 'B', 'B', 'A', 'A'], '7' => ['A', 'B', 'A', 'B', 'A', 'B'], '8' => ['A', 'B', 'A', 'B', 'B', 'A'], '9' => ['A', 'B', 'B', 'A', 'B', 'A']];
        $k = 0;
        $seq = '101';
        // left guard bar
