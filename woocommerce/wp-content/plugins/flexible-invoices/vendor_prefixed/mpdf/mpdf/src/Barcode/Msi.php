 ...
 */
class Msi extends \WPDeskFIVendor\Mpdf\Barcode\AbstractBarcode implements \WPDeskFIVendor\Mpdf\Barcode\BarcodeInterface
{
    /**
     * @param int $code
     * @param bool $checksum
    ...
    private function init($code, $checksum)
    {
        ...
        for ($i = 0; $i < $clen; ++$i) {
            ...
            if (!isset($chr[$digit])) {
                // invalid character
                throw new \WPDeskFIVendor\Mpdf\Barcode\BarcodeException(\sprintf('Invalid character "%s" in MSI barcode value', $digit));
            }
            $seq .= $chr[$digit];
        }
        $seq .= '1001';
