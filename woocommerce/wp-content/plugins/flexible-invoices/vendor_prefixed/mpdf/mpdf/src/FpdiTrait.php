...
trait FpdiTrait
{
    ...
    protected $k = \WPDeskFIVendor\Mpdf\Mpdf::SCALE;
    /**
    ...
    public function getImportedExternalPageLinks($pageNumber)
    {
        ...
        if ($annotations instanceof \WPDeskFIVendor\setasign\Fpdi\PdfParser\Type\PdfIndirectObjectReference) {
            $annotations = \WPDeskFIVendor\setasign\Fpdi\PdfParser\Type\PdfType::resolve($parser->getIndirectObject($annotations->value), $parser);
        }
        if ($annotations instanceof \WPDeskFIVendor\setasign\Fpdi\PdfParser\Type\PdfArray) {
            ...
            foreach ($annotations->value as $annotation) {
                $annotation = \WPDeskFIVendor\setasign\Fpdi\PdfParser\Type\PdfType::resolve($annotation, $parser)->value;
                ...
                $x1 = $getAttribute($position, 0) / \WPDeskFIVendor\Mpdf\Mpdf::SCALE;
                $y1 = $getAttribute($position, 1) / \WPDeskFIVendor\Mpdf\Mpdf::SCALE;
                $x2 = $getAttribute($position, 2) / \WPDeskFIVendor\Mpdf\Mpdf::SCALE;
                $y2 = $getAttribute($position, 3) / \WPDeskFIVendor\Mpdf\Mpdf::SCALE;
                $width = $x2 - $x1;
                $height = $y2 - $y1;
                $link = $annotation['A'] instanceof \WPDeskFIVendor\setasign\Fpdi\PdfParser\Type\PdfIndirectObjectReference ? \WPDeskFIVendor\setasign\Fpdi\PdfParser\Type\PdfType::resolve($annotation['A'], $parser)->value : $getAttribute($annotation, 'A');
                if (isset($link['URI'])) {
                    $links[] = ['x' => $x1, 'y' => $y1, 'width' => $width, 'height' => $height, 'url' => $getAttribute($link, 'URI')];
                }
            }
        }
        return $links;
     ...
     * @throws PdfTypeException
     */
    public function writePdfType(\WPDeskFIVendor\setasign\Fpdi\PdfParser\Type\PdfType $value)
    {
        if (!$this->encrypted) {
