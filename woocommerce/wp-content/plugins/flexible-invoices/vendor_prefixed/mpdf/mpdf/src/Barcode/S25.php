 ...
 */
class S25 extends \WPDeskFIVendor\Mpdf\Barcode\AbstractBarcode implements \WPDeskFIVendor\Mpdf\Barcode\BarcodeInterface
{
    /**
     * @param string $code
     * @param bool $checksum
    ...
    private function init($code, $checksum)
    {
        ...
        for ($i = 0; $i < $clen; ++$i) {
            ...
            if (!isset($chr[$digit])) {
                // invalid character
                throw new \WPDeskFIVendor\Mpdf\Barcode\BarcodeException(\sprintf('Invalid character "%s" in S25 barcode value', $digit));
            }
            $seq .= $chr[$digit];
        }
        $seq .= '1101011';
