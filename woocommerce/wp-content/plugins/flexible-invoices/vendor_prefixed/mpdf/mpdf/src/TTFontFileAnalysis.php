...
class TTFontFileAnalysis extends \WPDeskFIVendor\Mpdf\TTFontFile
{
    ...
    function extractCoreInfo($file, $TTCfontID = 0)
    {
        ...
        if ($version == 0x4f54544f) {
            throw new \WPDeskFIVendor\Mpdf\MpdfException('ERROR - NOT ADDED as Postscript outlines are not supported - ' . $file);
        }
        if ($version == 0x74746366) {
            if ($TTCfontID > 0) {
                ...
                throw new \WPDeskFIVendor\Mpdf\MpdfException("ERROR - Error parsing TrueType Collection - " . $file);
            }
        ...
        if ($format != 0 && $format != 1) {
            // mPDF 5.3.73
            throw new \WPDeskFIVendor\Mpdf\MpdfException("ERROR - NOT ADDED as Unknown name table format " . $format . " - " . $file);
        }
        $numRecords = $this->read_ushort();
        $string_data_offset = $name_offset + $this->read_ushort();
        $names = [1 => '', 2 => '', 3 => '', 4 => '', 6 => ''];
        ...
        if (!$names[1] && !$psName) {
            throw new \WPDeskFIVendor\Mpdf\MpdfException("ERROR - NOT ADDED as Could not find valid font name - " . $file);
        }
        $this->name = $psName;
        if ($names[1]) {
        ...
        if ($ver_maj != 1) {
            throw new \WPDeskFIVendor\Mpdf\MpdfException('ERROR - NOT ADDED as Unknown head table version ' . $ver_maj . '.' . $ver_min . " - " . $file);
        }
        $this->fontRevision = $this->read_ushort() . $this->read_ushort();
        $this->skip(4);
        $magic = $this->read_ulong();
        if ($magic != 0x5f0f3cf5) {
            throw new \WPDeskFIVendor\Mpdf\MpdfException('ERROR - NOT ADDED as Invalid head table magic ' . $magic . " - " . $file);
        }
        $this->skip(2);
        ...
        if (!$unicode_cmap_offset) {
            throw new \WPDeskFIVendor\Mpdf\MpdfException('ERROR - Font (' . $this->filename . ') NOT ADDED as it is not Unicode encoded, and cannot be used by mPDF');
        }
        $rtl = \false;
        $indic = \false;
        $cjk = \false;
        ...
        if ($format == 12) {
            ...
            for ($i = 0; $i < $nGroups; $i++) {
                ...
                if ($endCharCode > 0x590 && $endCharCode < 0x77f || $endCharCode > 0xfe70 && $endCharCode < 0xfeff || $endCharCode > 0xfb50 && $endCharCode < 0xfdff) {
                    $rtl = \true;
                }
                if ($endCharCode > 0x900 && $endCharCode < 0xdff) {
                    $indic = \true;
                }
            ...
            for ($n = 0; $n < $segCount; $n++) {
                if ($endCount[$n] > 0x590 && $endCount[$n] < 0x77f || $endCount[$n] > 0xfe70 && $endCount[$n] < 0xfeff || $endCount[$n] > 0xfb50 && $endCount[$n] < 0xfdff) {
                    $rtl = \true;
                }
                if ($endCount[$n] > 0x900 && $endCount[$n] < 0xdff) {
                    $indic = \true;
                }
