 ...
 */
class Code93 extends \WPDeskFIVendor\Mpdf\Barcode\AbstractBarcode implements \WPDeskFIVendor\Mpdf\Barcode\BarcodeInterface
{
    /**
     * @param string $code
     */
    ...
    private function init($code)
    {
        ...
        for ($i = 0; $i < $clen; ++$i) {
            if (\ord($code[$i]) > 127) {
                throw new \WPDeskFIVendor\Mpdf\Barcode\BarcodeException(\sprintf('Invalid character "%s" in Code93 barcode value', $code[$i]));
            }
        ...
        for ($i = 0; $i < $clen; ++$i) {
            ...
            if (!isset($chr[$char])) {
                // invalid character
                throw new \WPDeskFIVendor\Mpdf\Barcode\BarcodeException('Invalid CODE93 barcode value');
            }
            for ($j = 0; $j < 6; ++$j) {
                if ($j % 2 == 0) {
                    $t = \true;
        ...
        $bararray['bcode'][$k] = ['t' => \true, 'w' => 1, 'h' => 1, 'p' => 0];
        $bararray['maxw'] += 1;
        $this->data = $bararray;
    }
    /**
