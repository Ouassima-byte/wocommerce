 ...
 */
class Code11 extends \WPDeskFIVendor\Mpdf\Barcode\AbstractBarcode implements \WPDeskFIVendor\Mpdf\Barcode\BarcodeInterface
{
    /**
     * @param string $code
    ...
    private function init($code, $printRatio)
    {
        ...
        for ($i = 0; $i < $len; ++$i) {
            if (!isset($chr[$code[$i]])) {
                throw new \WPDeskFIVendor\Mpdf\Barcode\BarcodeException(\sprintf('Invalid character "%s" in CODE11 barcode value', $code[$i]));
            }
            $seq = $chr[$code[$i]];
            for ($j = 0; $j < 6; ++$j) {
                if ($j % 2 == 0) {
