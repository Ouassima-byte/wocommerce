...
class Svg
{
    ...
    private $inDefs;
    public function __construct(\WPDeskFIVendor\Mpdf\Mpdf $mpdf, \WPDeskFIVendor\Mpdf\Otl $otl, \WPDeskFIVendor\Mpdf\CssManager $cssManager, \WPDeskFIVendor\Mpdf\Image\ImageProcessor $imageProcessor, \WPDeskFIVendor\Mpdf\SizeConverter $sizeConverter, \WPDeskFIVendor\Mpdf\Color\ColorConverter $colorConverter, \WPDeskFIVendor\Mpdf\Language\LanguageToFontInterface $languageToFont, \WPDeskFIVendor\Mpdf\Language\ScriptToLanguageInterface $scriptToLanguage)
    {
        $this->mpdf = $mpdf;
        $this->otl = $otl;
    ...
    function svgImage($attribs)
    {
        ...
        $info = $this->imageProcessor->getImage($srcpath, \true, \false, $orig_srcpath, $this->mpdf->interpolateImages);
        if (!$info) {
            return;
        }
    ...
    function svgGradient($gradient_info, $attribs, $element)
    {
        ...
        if ($element == 'rect') {
            ...
            $pts = \preg_split('/[ ,]+/', \trim($attribs['points']));
            $maxr = $maxb = 0;
            $minl = $mint = 999999;
            ...
            if ($spread == 'R' || $spread == 'F') {
                ...
                while ($inside) {
                    ...
                    for ($i = 0; $i < $ns; $i++) {
                        ...
                        $px2 = $px1 + \cos($alpha);
                        $py2 = $py1 + \sin($alpha);
                        $res1 = $this->testIntersect($px1, $py1, $px2, $py2, 0, 0, 0, 1);
                        // $x=0 vert axis
                        $res2 = $this->testIntersect($px1, $py1, $px2, $py2, 1, 0, 1, 1);
                        // $x=1 vert axis
                        $res3 = $this->testIntersect($px1, $py1, $px2, $py2, 0, 0, 1, 0);
                        // $y=0 horiz axis
                        $res4 = $this->testIntersect($px1, $py1, $px2, $py2, 0, 1, 1, 1);
                        // $y=1 horiz axis
    ...
    function svgOffset($attribs)
    {
        ...
        if (isset($this->svg_attribs['viewBox'])) {
            $vb = \preg_split('/\\s+/is', \trim($this->svg_attribs['viewBox']));
            if (\count($vb) == 4) {
        ...
        if (isset($attribs['width']) && $attribs['width']) {
            $svg_w = $this->sizeConverter->convert($attribs['width']);
            // mm (interprets numbers as pixels)
        }
        if (isset($attribs['height']) && $attribs['height']) {
            $svg_h = $this->sizeConverter->convert($attribs['height']);
            // mm
        }
        ///*
        // mPDF 5.0.005
    ...
    function svgStyle($critere_style, $attribs, $element)
    {
        ...
        if (\substr_count($critere_style['fill'], 'url') > 0 && $element != 'line') {
            ...
            if ($col) {
                ...
                // CMYKa
                $path_style .= $this->mpdf->SetFColor($col, \true) . ' ';
                $style .= 'F';
            }
        ...
        if (\substr_count($critere_style['stroke'], 'url') > 0) {
            ...
            */
            ...
            if ($col) {
                ...
                // CMYKa
                $path_style .= $this->mpdf->SetDColor($col, \true) . ' ';
                $style .= 'D';
        ...
        if (isset($critere_style['fill-opacity'])) {
            ...
            $this->mpdf->extgstates[$gs]['fo'] = \true;
    ...
    function ConvertSVGSizePts($size = 5)
    {
        ...
        //Return as pts
        $size = $this->sizeConverter->convert($size, $maxsize, \false, \true) * 72 / 25.4;
        return $size;
    }
    //
    ...
    function svgPolyline($arguments, $ispolyline = \true)
    {
        if ($ispolyline) {
            $xbase = $arguments[0];
            $ybase = -$arguments[1];
        ...
        if (isset($this->txt_data[2])) {
            ...
            if (isset($this->svg_font[$current_style['font-family']])) {
                ...
                if (isset($current_style['fill']) && $current_style['fill'] != 'none') {
                    ...
                    $fillstr = $this->mpdf->SetFColor($col, \true);
                    $render = "0";
                    // Fill (only)
                    $op = 'f';
                }
                ...
                if ($stroke_width > 0 && $current_style['stroke'] != 'none') {
                    ...
                    if ($scol) {
                        $strokestr .= $this->mpdf->SetDColor($scol, \true) . ' ';
                    }
            ...
            if ($this->mpdf->useKerning) {
                if ($this->mpdf->CurrentFont['haskernGPOS']) {
                    ...
                    $textvar = $textvar | \WPDeskFIVendor\Mpdf\Css\TextVars::FC_KERNING;
                }
            }
            ...
            $this->mpdf->CurrentFont['used'] = \true;
            $sw = $this->mpdf->GetStringWidth($txt, \true, $OTLdata, $textvar);
            ...
            // Align X-shift
            $path_cmd .= $this->mpdf->Text($pdfx, $pdfy, $txt, $OTLdata, $textvar, $aixextra, 'SVG', \true);
            $path_cmd .= " Q\n";
            unset($this->txt_data[0], $this->txt_data[1], $this->txt_data[2]);
    ...
    function svgDefineTxtStyle($critere_style)
    {
        ...
        if (isset($critere_style['font-family']) && $critere_style['font-family'] != 'inherit') {
            ...
            foreach ($aux_fontlist as $f) {
                ...
                if (!$this->mpdf->usingCoreFont && \in_array($v, $this->mpdf->available_unifonts) || $this->mpdf->usingCoreFont && \in_array($v, ['courier', 'times', 'helvetica', 'arial']) || \in_array($v, ['sjis', 'uhc', 'big5', 'gb']) || isset($this->svg_font[$v][$svgfontstyle])) {
                    // mPDF 6
    ...
    function markScriptToLang($html)
    {
        ...
        foreach ($a as $i => $e) {
            if ($i % 2 == 0) {
                $e = \WPDeskFIVendor\Mpdf\Utils\UtfString::strcode2utf($e);
                ...
                for ($sch = 0; $sch <= $subchunk; $sch++) {
                    if (isset($chardata[$sch])) {
                        ...
                        for ($j = 0; $j < \count($chardata[$sch]); $j++) {
                            $s .= \WPDeskFIVendor\Mpdf\Utils\UtfString::code2utf($chardata[$sch][$j]['uni']);
                        }
                        ...
                        if ($scriptblocks[$sch] == \WPDeskFIVendor\Mpdf\Ucdn::SCRIPT_LATIN && $this->mpdf->autoVietnamese && \preg_match("/([" . $this->scriptToLanguage->getLanguageDelimiters('viet') . "])/u", $s)) {
                            $lang = "vi";
                        } elseif ($scriptblocks[$sch] == \WPDeskFIVendor\Mpdf\Ucdn::SCRIPT_ARABIC && $this->mpdf->autoArabic) {
                            if (\preg_match("/[" . $this->scriptToLanguage->getLanguageDelimiters('sindhi') . "]/u", $s)) {
                                ...
                                $lang = "fa";
                            } elseif ($this->mpdf->baseScript != \WPDeskFIVendor\Mpdf\Ucdn::SCRIPT_ARABIC && $this->scriptToLanguage->getLanguageByScript($scriptblocks[$sch])) {
    ...
    function xml_svg2pdf_start($parser, $name, $attribs)
    {
        ...
        switch (\strtolower($name)) {
            ...
            case 'svg':
                ...
                break;
            case 'path':
                $path = \WPDeskFIVendor\Mpdf\Utils\Arrays::get($attribs, 'd', '');
            ...
            case 'text':
                ...
                if ($this->mpdf->svgClasses && isset($attribs['class']) && $attribs['class']) {
                    $classes = \preg_split('/\\s+/', \trim($attribs['class']));
                ...
                \array_push($this->svg_style, $array_style);
                $this->txt_data = [];
                $x = isset($attribs['x']) ? $this->ConvertSVGSizePixels($attribs['x'], 'x') : 0;
                // mPDF 5.7.4
                $y = isset($attribs['y']) ? $this->ConvertSVGSizePixels($attribs['y'], 'y') : 0;
                // mPDF 5.7.4
                $x += isset($attribs['dx']) ? $this->ConvertSVGSizePixels($attribs['dx'], 'x') : 0;
                // mPDF 5.7.4
                $y += isset($attribs['dy']) ? $this->ConvertSVGSizePixels($attribs['dy'], 'y') : 0;
                // mPDF 5.7.4
                $this->txt_data[0] = $x;
                // mPDF 5.7.4
                $this->txt_data[1] = $y;
                ...
                // mPDF 5.7.4
                $this->textXorigin = $this->txt_data[0];
                // mPDF 5.7.4
                $this->textYorigin = $this->txt_data[1];
                // mPDF 5.7.4
                $this->textjuststarted = \true;
                // mPDF 5.7.4
                break;
            // mPDF 5.7.4
            case 'tspan':
