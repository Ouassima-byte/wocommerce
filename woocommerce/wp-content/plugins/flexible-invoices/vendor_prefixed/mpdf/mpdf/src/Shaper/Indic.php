...
namespace WPDeskFIVendor\Mpdf\Shaper;
...
class Indic
{
    ...
    public static function insert_dotted_circles(&$info, $dottedcircle)
    {
        ...
        if (isset($info[$idx])) {
            throw new \WPDeskFIVendor\Mpdf\MpdfException('Unexpected error occured in Indic processing');
        }
        ...
        if ($scriptblock != \WPDeskFIVendor\Mpdf\Ucdn::SCRIPT_KHMER) {
            if (\count($GSUBdata['pref']) && $base + 2 < $end) {
                ...
                for ($i = $base + 1; $i + 1 < $end; $i++) {
                    ...
                    if (isset($GSUBdata['pref'][$info[$i + 1]['uni']]) && self::is_halant_or_coeng($info[$i]) && self::is_ra($info[$i + 1]['uni']) || $is_old_spec && isset($GSUBdata['pref'][$info[$i]['uni']]) && self::is_halant_or_coeng($info[$i + 1]) && self::is_ra($info[$i]['uni'])) {
                        $info[$i++]['mask'] |= self::FLAG(self::PREF);
                        $info[$i++]['mask'] |= self::FLAG(self::PREF);
                        break;
                    }
