...
use WPDeskFIVendor\WPDesk\Helper\HelperRemover;
use WPDeskFIVendor\WPDesk\Helper\PrefixedHelperAsLibrary;
use WPDeskFIVendor\WPDesk\License\PluginRegistrator;
use WPDeskFIVendor\WPDesk\Plugin\Flow\Initialization\ActivationTrait;
use WPDeskFIVendor\WPDesk\Plugin\Flow\Initialization\BuilderTrait;
use WPDeskFIVendor\WPDesk\Plugin\Flow\Initialization\PluginDisablerByFileTrait;
use WPDeskFIVendor\WPDesk\Plugin\Flow\Initialization\InitializationStrategy;
use WPDeskFIVendor\WPDesk\PluginBuilder\Plugin\ActivationAware;
 ...
 */
class SimplePaidStrategy implements \WPDeskFIVendor\WPDesk\Plugin\Flow\Initialization\InitializationStrategy
{
    ...
    public function run_init(\WPDeskFIVendor\WPDesk_Plugin_Info $plugin_info)
    {
        ...
        \add_action('plugins_loaded', function () use($registrator) {
            $this->init_helper();
            $is_plugin_subscription_active = $registrator instanceof \WPDeskFIVendor\WPDesk\License\PluginRegistrator && $registrator->is_active();
            if ($this->plugin instanceof \WPDeskFIVendor\WPDesk\PluginBuilder\Plugin\ActivationAware && $is_plugin_subscription_active) {
    ...
    private function register_plugin()
    {
        if (\apply_filters('wpdesk_can_register_plugin', \true, $this->plugin_info)) {
            $registrator = new \WPDeskFIVendor\WPDesk\License\PluginRegistrator($this->plugin_info);
    ...
    private function prevent_older_helpers()
    {
        ...
        if (\apply_filters('wpdesk_can_remove_old_helper_hooks', \true, $this->plugin_info)) {
            (new \WPDeskFIVendor\WPDesk\Helper\HelperRemover())->hooks();
        }
    }
    /**
    ...
    {
        (new \WPDeskFIVendor\WPDesk\Plugin\Flow\Initialization\PluginDisablerByFileTrait('wpdesk-helper/wpdesk-helper.php'))->disable();
    }
}
