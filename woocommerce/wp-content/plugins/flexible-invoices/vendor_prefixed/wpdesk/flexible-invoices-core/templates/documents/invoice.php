...
$has_vat = !$invoice->get_total_tax() || (float) $invoice->get_total_tax() !== 0.0;
...
?>css/front.css" rel="stylesheet" type="text/css"
          media="screen,print"/>
    <?php 
\do_action('fi/core/template/invoice');
?>
</head>
<body>
...
?>
                ...
                <td id="dates">
                    <p><?php 
echo \trim($translator::translate_meta('inspire_invoices_invoice_date_of_sale_label', \__('Date of sale', 'flexible-invoices-woocommerce')));
?>: <strong><?php 
echo $invoice->get_date_of_sale();
?></strong></p>
                    <p><?php 
echo \__('Issue date', 'flexible-invoices-woocommerce');
?>: <strong><?php 
echo $invoice->get_date_of_issue();
?></strong></p>
                    <?php 
if ($invoice->get_date_of_pay() > 0) {
    ?>
                        <p><?php 
    echo \__('Due date', 'flexible-invoices-woocommerce');
    ?>: <strong><?php 
...
?>
                    <?php 
$paymentMethod = $invoice->get_payment_method_name();
?>
                    <?php 
if (!empty($paymentMethod)) {
    ?>
                        <p><?php 
    echo \__('Payment method', 'flexible-invoices-woocommerce');
    ?>: <strong><?php 
    echo $paymentMethod;
    ?></strong></p>
                    <?php 
...
?>
        ...
        <div id="companies">
            <div class="seller">
                <p class="title"><?php 
echo \__('Seller', 'flexible-invoices-woocommerce');
?>:</p>
...
?>
                ...
                <p class="details"><?php 
echo \nl2br(!empty($owner->get_address()) ? $owner->get_address() : '');
?></p>
...
if (!empty($owner->get_bank_name())) {
    ?>
                    <p><?php 
    echo \__('Bank', 'flexible-invoices-woocommerce');
    ?>: <?php 
...
if (!empty($owner->get_bank_account_number())) {
    ?>
                    <p><?php 
    echo \__('Account number', 'flexible-invoices-woocommerce');
    ?>
                        : <?php 
...
?>
            ...
            <div class="buyer">
                <p class="title"><?php 
echo \__('Buyer', 'flexible-invoices-woocommerce');
?>:</p>
...
if (!empty($client->get_city())) {
    ...
    ?>
                        <span><?php 
    echo !empty($client->get_country()) ? $client->get_country() : '';
    ?></span>
                    <?php 
}
?>
...
?></p>
    ...
    <table cellpadding="0" cellspacing="0">
        <thead>
        <?php 
$price_label = $hideVat ? \__('Price', 'flexible-invoices-woocommerce') : \__('Net price', 'flexible-invoices-woocommerce');
$amount_label = $hideVat ? \__('Amount', 'flexible-invoices-woocommerce') : \__('Net amount', 'flexible-invoices-woocommerce');
?>

        <tr>
            <th><?php 
echo \__('#', 'flexible-invoices-woocommerce');
?></th>
            <th width="30%"><?php 
echo \__('Name', 'flexible-invoices-woocommerce');
?></th>
            <?php 
if (!$pkwiuEmpty) {
    ?>
                <th><?php 
    echo \__('SKU', 'flexible-invoices-woocommerce');
    ?></th>
            <?php 
}
?>
            <th><?php 
echo \__('Quantity', 'flexible-invoices-woocommerce');
?></th>
            <th><?php 
echo \__('Unit', 'flexible-invoices-woocommerce');
?></th>
            <th><?php 
echo $price_label;
?></th>
...
if (!$hideVat) {
    ?>
                <th><?php 
    echo \__('Tax rate', 'flexible-invoices-woocommerce');
    ?></th>
                <th><?php 
    echo \__('Tax amount', 'flexible-invoices-woocommerce');
    ?></th>
                <th><?php 
    echo \__('Gross amount', 'flexible-invoices-woocommerce');
    ?></th>
            <?php 
}
?>
...
echo \__('Total', 'flexible-invoices-woocommerce');
?></td>
...
if (!$hideVat) {
    ...
    foreach ($total_tax_net_price as $taxType => $price) {
        ...
        ?>
                    <td class="sum-title"><?php 
        echo \__('Including', 'flexible-invoices-woocommerce');
        ?></td>
                    <td class="number"><?php 
...
?>
    ...
    <?php 
\do_action('flexible_invoices_before_total', $invoice, $products, $client);
?>
    <?php 
$total_section = '<table class="totals">
    			<tbody>
    				<tr>
    					<td id="total_price" width="33.3%">' . \esc_html__('Total', 'flexible-invoices-woocommerce') . ': <strong>' . $helper->string_as_money($invoice->get_total_gross()) . '</strong></td>
						<td id="paid_price" width="33.3%">' . \esc_html__('Paid', 'flexible-invoices-woocommerce') . ': <strong>' . $helper->string_as_money($invoice->get_total_paid()) . '</strong></td>
						<td id="due_price" width="33.3%">' . \esc_html__('Due', 'flexible-invoices-woocommerce') . ': <strong>' . $helper->string_as_money(\WPDeskFIVendor\WPDesk\Library\FlexibleInvoicesCore\Helpers\CalculateTotals::calculate_due_price($invoice->get_total_gross(), $invoice->get_total_paid())) . '</strong></td>
    				</tr>
    			</tbody>
    		</table>';
...
?>
    <?php 
\do_action('flexible_invoices_after_total', $invoice, $products, $client);
?>
...
if ($settings->get('show_signatures') === 'yes') {
    ...
    ?>
                ...
                <tr>
                    <td>
                        <p><?php 
    echo \__('Buyer signature', 'flexible-invoices-woocommerce');
    ?></p>
                    </td>
                    ...
                    <td>
                        <p><?php 
    echo \__('Seller signature', 'flexible-invoices-woocommerce');
    ?></p>
                    </td>
                </tr>
            </table>
...
if (!empty($note)) {
    ?>
        <div id="footer">
            <p><strong><?php 
    echo \__('Notes', 'flexible-invoices-woocommerce');
    ?></strong></p>
            <p><?php 
...
?>
    ...
    <?php 
\do_action('flexible_invoices_after_invoice_notes', $client_country, $hideVat, $hideVatNumber, $invoice);
?>
    <?php 
...
if ($invoice->get_show_order_number()) {
    ...
    ?>
        <p><?php 
    echo \__('Order number', 'flexible-invoices-woocommerce');
    ?>: <?php 
