...
class DownloadMenuPage implements \WPDeskFIVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    {
        return [(new \WPDeskFIVendor\WPDesk\Forms\Field\Header())->set_label(\__('Download Invoices', 'flexible-invoices'))->set_description(\__('If the download fails select smaller date range and try again.', 'flexible-invoices')), (new \WPDeskFIVendor\WPDesk\Library\FlexibleInvoicesCore\Settings\Fields\GroupedFields())->set_name('grouped_field')->set_grouped_fields([(new \WPDeskFIVendor\WPDesk\Forms\Field\InputTextField())->set_name('start_date')->set_label(\__('From:', 'flexible-invoices'))->add_class('medium-text datepicker hs-beacon-search')->set_default_value(\date('Y-m-d', \strtotime('NOW - 1 months')))->set_attribute('data-beacon_search', 'Download'), (new \WPDeskFIVendor\WPDesk\Forms\Field\InputTextField())->set_name('end_date')->set_label(\__('To:', 'flexible-invoices'))->add_class('medium-text datepicker hs-beacon-search')->set_default_value(\date('Y-m-d', \strtotime('NOW')))->set_attribute('data-beacon_search', 'Download')]), (new \WPDeskFIVendor\WPDesk\Library\FlexibleInvoicesCore\Settings\Fields\FixedSubmitField())->set_name('download_documents')->set_label(\__('Download', 'flexible-invoices'))->add_class('button-primary'), (new \WPDeskFIVendor\WPDesk\Forms\Field\NoOnceField(self::NONCE_ACTION))->set_name(self::NONCE_NAME)];
    }
    /**
     * @return string
     */
    ...
    {
        return \__('Download', 'flexible-invoices');
    }
}
