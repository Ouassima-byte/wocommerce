...
namespace WPDeskFIVendor\WPDesk\Helper;

use Psr\Log\LoggerInterface;
use WPDeskFIVendor\WPDesk\Helper\Debug\LibraryDebug;
use WPDeskFIVendor\WPDesk\Helper\Integration\LicenseIntegration;
use WPDeskFIVendor\WPDesk\Helper\Integration\LogsIntegration;
use WPDeskFIVendor\WPDesk\Helper\Integration\SettingsIntegration;
use WPDeskFIVendor\WPDesk\Helper\Integration\TrackerIntegration;
use WPDeskFIVendor\WPDesk\Helper\Logs\LibraryInfoProcessor;
use WPDeskFIVendor\WPDesk\Helper\Page\LibraryDebugPage;
 ...
 */
class PrefixedHelperAsLibrary implements \WPDeskFIVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    {
        $helper_info = new \WPDeskFIVendor\WPDesk\Helper\HelperRemoveInfo();
    ...
    private function initialize()
    {
        ...
        $subscription_integration = new \WPDeskFIVendor\WPDesk\Helper\Integration\LicenseIntegration();
        $subscription_integration->hooks();
        $settingsPage = new \WPDeskFIVendor\WPDesk\Helper\Page\SettingsPage();
        $settings_integration = new \WPDeskFIVendor\WPDesk\Helper\Integration\SettingsIntegration($settingsPage);
        $tracker_integration = new \WPDeskFIVendor\WPDesk\Helper\Integration\TrackerIntegration($settingsPage);
        $logger_integration = new \WPDeskFIVendor\WPDesk\Helper\Integration\LogsIntegration($settingsPage);
        ...
        (new \WPDeskFIVendor\WPDesk\Helper\UpgradeSoonNotice())->show_info_about_upgrade_if_old_env();
        $this->clean_wpdesk_menu();
        $library_debug_info = new \WPDeskFIVendor\WPDesk\Helper\Debug\LibraryDebug();
        (new \WPDeskFIVendor\WPDesk\Helper\Page\LibraryDebugPage($library_debug_info))->hooks();
        self::$logger->pushProcessor(new \WPDeskFIVendor\WPDesk\Helper\Logs\LibraryInfoProcessor($library_debug_info));
